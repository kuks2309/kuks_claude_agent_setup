...
#   6) 설치 성공 시 <타깃>/docs/claude_guideline/INSTALLED.md 에 자기 행 기록(커밋·날짜·인자)
#
...
# registry.md(docs/debt/registry.md)는 기존 있으면 보존되는 사용자 소유 문서라 제외.
drift_pairs() {
  printf '%s\t%s\n' "$SRC/debt.md" "$DEST/debt.md"
  local f
  for f in "$SRC/domains/"*.md; do
    [ -f "$f" ] || continue
    printf '%s\t%s\n' "$f" "$DEST/domains/$(basename "$f")"
  done
  for f in "$SRC/checks/"*.sh; do
    [ -f "$f" ] || continue
    printf '%s\t%s\n' "$f" "$DEST/checks/$(basename "$f")"
  done
  if [ -f "$SRC/.pre-commit-config.yaml" ]; then
    printf '%s\t%s\n' "$SRC/.pre-commit-config.yaml" "$DEST/pre-commit-config.sample.yaml"
  fi
  if [ -d "$SRC/ci" ]; then
    for f in "$SRC/ci/"*.yml; do
      [ -f "$f" ] || continue
      printf '%s\t%s\n' "$f" "$DEST/ci/$(basename "$f")"
    done
  fi
