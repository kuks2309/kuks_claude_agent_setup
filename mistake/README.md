...
- **entry-lint 이빨**(`checks/entry-lint.sh`) = 형식·closure 규칙(단일 frontmatter·id/파일명·type↔category 정합·owner 규칙·TBD(To Be Determined) 금지·5 절 순서·open 7 일 시한·retracted 기각 각주/오염 목록/청소 시한·교차 파일 id 중복·반영 자산 실재) 기계 검출. pre-commit·CI(Continuous Integration) 에 연결하면 커밋 게이트로 승격 가능.
