...
**게이트 동작**: 대상 코드 파일을 등재한 **최근접 조상 모듈의 표**(모듈 로컬 = 권위본)를 이번 세션에 읽었는지 검사한다. 등재 판정은 표의 위치 컬럼 형식(`backend.py:315-349`)에 맞춘 `파일명:줄` 앵커. `docs/claude_guideline/**`(설치된 규칙 문서)는 표 후보에서 제외. **표가 없으면 통과**가 기본값이고(미도입 프로젝트 전면 차단 시 훅이 꺼져 강제력 0 회귀), `CODING_GATE=strict` 로 차단 전환.
