...
- **타 세션의 수정 내용에는 관여하지 않는다** — 수정·정리뿐 아니라 **진단·평가·처리 제안**(커밋할지, 정리할지, 살릴지)도 하지 않는다. 남의 미완 작업은 그 세션의 판단이 아직 끝나지 않은 상태이고, 밖에서 본 스냅샷으로는 완성도·의도를 알 수 없다. 노출은 §4 handoff 채널이 전담한다(§별도 규칙).
...
**종료 보고 경계**: 종료 보고(커밋 사유 보고 포함)는 **이 세션의 산출물·커밋·미커밋 목록만** 담는다.

**타 세션의 수정 내용은 보고 대상이 아니다** — 목록화·진단·평가도, "이걸 어떻게 할지 정해달라"는 결정 요청도 하지 않는다. 그 작업의 노출은 §4 handoff 와 시작 주입이 이미 전담하므로 모델이 중복해서 올릴 이유가 없고, 밖에서 본 스냅샷으로 남의 미완 작업의 완성도·의도를 판단할 수도 없다.

유일한 예외는 **그 상태가 이 세션 작업을 실제로 막을 때**다. 그때도 *막힌 사실 1줄*로 끝내고(무엇을 어떻게 하라는 제안·선택지 제시 금지), 상세는 사용자가 물을 때만 답한다. "사용자 결정이 필요해 보인다"는 모델의 자체 판단은 예외 사유가 **아니다** — 그 재량이 곧 관여의 통로가 된다(실측: 잔존 워크트리 1건을 "결정 필요"로 분류해 문단째 보고했다).

격리 규칙 준수를 위해 수행한 트리 진단은 수단이지 보고 대상이 아니다 — 진단 내용을 보고에 그대로 쏟지 않는다.
...
6. 종료·커밋 보고는 **이 세션 작업만** — **타 세션의 수정 내용에는 관여하지 않는다**(수정·정리는 물론 진단·평가·처리 제안·결정 요청도 금지). 노출은 handoff 채널 소관. 예외는 그 상태가 이 세션 작업을 실제로 막을 때뿐이며 그때도 막힌 사실 1줄.
