...
- **조작 전 대상 창을 앞으로 올리고(raise/activate) 재캡처로 시각 확인한다.**
  상위에 다른 창이 덮여 있으면(z-order) 입력이 엉뚱한 창에 들어갈 수 있다.
  `capture_screen.py --mode window --window-id <id>`(캡처 전 자동 raise) 또는
  `xdotool windowactivate <id>` 를 쓴다.
