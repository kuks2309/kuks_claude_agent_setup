...
```bash
SB=$(mktemp -d)
<repo>/git_workflow/install.sh "$SB"        # 1회
<repo>/git_workflow/install.sh "$SB"        # 2회 (멱등)
grep -c 'kuks_agent_setup:git_workflow' "$SB/CLAUDE.md"      # 기대 1
ls "$SB/docs/claude_guideline/git_workflow/"
```
...
| 케이스 | 측정 | 기대 | 판정 |
|--------|------|------|------|
| 규칙 파일 복사 | — | docs/claude_guideline/git_workflow/ 존재 | ⏳ |
| CLAUDE.md marker | — | 1회 | ⏳ |
| 재실행 멱등 | — | 2회째 "스킵" | ⏳ |
